...
class Logtail {
  ...
  async log(message, level = LogLevel.Info, context = {}) {
    ...
    let log = Object.assign(Object.assign({
      dt: new Date,
      level
    }, context), message instanceof Error ? import_serialize_error.serializeError(message) : { message });
...
function getCallingFrame(logtail, stackContextHint) {
  for (let fn of [logtail.warn, logtail.error, logtail.info, logtail.debug, logtail.log]) {
    ...
    if ((stack === null || stack === undefined ? undefined : stack.length) > 0)
...
class LogtailTransport extends import_winston_transport.default {
  ...
  log(info, cb) {
    ...
    this._logtail.log(message, level, meta, stackContextHint);
    cb();
  }
}
// src/helper/logger.ts
...
var logtail = new Node(process.env.LOGTAIL_API_KEY || "qLeeazh2QzV47U2f7EnoMhT8");
